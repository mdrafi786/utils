...
class Cryptography {
    ...
    companion object {
        ...
        private const val KEY_ALIAS = "Craterzone_Alias"

        //  update these bytes to be random for IV of encryption
        ...
        private const val SHARED_PREFERENCE_NAME = "Craterzone_Pref"
        private const val ENCRYPTED_KEY_NAME = "Craterzone"
        private val LOG_TAG = Cryptography::class.java.name
    }

    private var mContext: Context? = null

    private val sKeyInitLock = Any()
    ...
    private fun initKeys() {
        ...
        if (!keyStore.containsAlias(KEY_ALIAS)) {
            ...
            try {
                ...
                Log.e(LOG_TAG, "Failed to get key store entry")
            } catch (e: UnrecoverableKeyException) {
                Log.e(LOG_TAG, "Failed to get key store entry")
            }
            if (!keyValid) {
                synchronized(sKeyInitLock) {
